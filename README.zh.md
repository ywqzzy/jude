...
**对象存储 IO**(`jude.storage`):基于 fsspec 读写 parquet/csv/json(含 `.gz`),任意 URL —— `s3://` / `gs://` / 本地 / `memory://`,数据源与 sink 直接指向 S3/GCS/MinIO(凭证走 `storage_options`)。**WARC/WET 摄入**(`jude.warc`):把 CommonCrawl 归档流式读入 Arrow。
