...
| 编号 | 痛点 | 视角 | 状态 |
|---|---|---|---|
| L0.1 | 只支持本地 FS，无 S3/GCS/OSS（数据都在对象存储，PB 级） | both | ⬜ |
...
### L2 —— 去重（预训练最硬的问题）
| L2.1 | 全局模糊去重（MinHash-LSH 跨**整语料**万亿文档；当前 driver 侧 CC 物化全量 rid，扛不住万亿） | pre | 🔧(小规模对) |
| L2.2 | 全局精确子串去重（`substring_dedup` 现单机顺序、全局 hash set 在一进程） | pre | 🔧(单机) |
...
| L3.2 | **模型质量分类器作为一等 stage**（FineWeb-edu/DCLM：fastText/小 BERT，CPU 可跑） | both | ⬜ |
...
| L4.1 | tokenizer 接入（HF tokenizers / tiktoken），token 计数 | both | ⬜ |
| L4.2 | sequence packing（拼接+按 seq_len 切、doc 边界、EOS、attention mask） | both | ⬜ |
| L4.3 | token-shard 写出 + 索引（Megatron `.bin/.idx` 或 Lance token 列）| both | ⬜ |
| L4.4 | token 级全局 shuffle（PB token 跨 shard 洗牌，落盘/对象存储，非内存） | both | 🔧(内存 shuffle 有) |
| L4.5 | tokenizer-aware 长度阈值（质量门用 token 数而非词数，C15） | both | ⬜ |

### L5 —— 混合 / 血缘 / 可复现
| L5.1 | token 比例配比 + 上/下采样 + 域权重（DoReMi 式）+ epoch 控制 | pre | 🔧(blend 有) |
| L5.2 | provenance 列（每条 token 来自哪个源、过了哪些 filter/版本） | both | ⬜ |
| L5.3 | **pipeline 配置 hash → 数据版本 → checkpoint** 血缘绑定（Lance 版本化是地基） | both | 🔧 |
| L5.4 | 语料 profiling（HLL 基数、长度/语言/dup 率 sketch，PB 级近似统计） | both | ⬜ |

### Cross-cutting —— 健壮性 & 规模
| X.1 | 长作业容错：**用 stage-based + Ray lineage 内建容错**，当前 `_JudeWorker` 是裸 `@ray.remote`（`max_restarts=0`，容错**没开**） | both | 🔧(机制在,没配) |
...
- **X.1 长作业容错 —— 用 Ray 内建 lineage 容错，不自建 checkpoint 系统**：collect 路径本就是 stage-based
  （task→ObjectRef→`ray.get`），Ray 的 **object reconstruction + task/actor 重试**天然能恢复丢失的中间结果——
  当前只是**没开**。要做的很小：
  1. `_JudeWorker` 加 `max_restarts` + `max_task_retries`（actor 死了自动重启；它的状态只有可重建的
     缓存，重启即重建，无需持久化）；task 提交带 `max_retries`；
  2. **确定性审计**：lineage 重算要求 task 确定性——jude 的 shuffle/salt 用确定性哈希（`_det_hash`、
     `row_number`），已满足；标注/守护"stage 函数必须确定"；
  3. 源读durable（Lance/S3）→ lineage 递归到底是重读存储，天然可重建；
  4. **可选的粗粒度 durable 物化**：超长 DAG 在 shuffle 边界 `write_lance` 落一次，失败从最近物化 stage
     重算而非从源头——这才是"checkpoint"，但极廉价（复用 Lance 写）。
  - **流式/pipelined 生成器路径不做 FT**（中途死了丢位置）——FT-critical 的长作业走 stage-based 执行器。
  - *测试：`ray.cluster_utils` 杀 worker，验证 actor 重启 + lineage 重算 → 结果与无故障一致；确定性对拍。*
