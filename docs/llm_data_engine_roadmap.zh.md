...
| 编号 | 痛点 | 视角 | 状态 |
|---|---|---|---|
| L0.1 | 只支持本地 FS,无 S3/GCS/OSS(数据都在对象存储,PB 级) | both | ✅(jude.storage fsspec 层,**真 S3 读写往返已测**(moto/MinIO 等价);Lance 写走自身 storage_options;`[storage]` extra) |
| L0.2 | 无 WARC/WAT/WET 读（CommonCrawl 是 WARC,预训练管线的入口） | pre | 🔧(jude.warc 读 WARC→Arrow,fsspec 任意 URL;warcio 可选依赖) |
| L0.3 | 输出只 `collect()` 成一张表,无**流式写 sink**（PB 输出攒不进内存） | both | 🔧(RelationPipeline.write_streaming:边产边写 Lance/parquet 分片,输入输出双向有界) |
| L0.4 | 无压缩分片流式读（`.jsonl.gz`/`.warc.gz` 千万分片） | pre | 🔧(单机流有) |

### L1 —— 抽取 & 规范化
| L1.1 | HTML→正文抽取（trafilatura/resiliparse 式 boilerplate 去除） | pre | 🔧(curate.html_to_text 内置零依赖 strip + resiliparse/trafilatura 可选) |
| L1.2 | 编码/乱码修复（ftfy 式 mojibake fix）、Unicode NFC 规范化 | both | ✅(curate.fix_encoding + normalize_unicode) |
| L1.3 | 语言识别（heuristic 已做；缺 fastText lid.176 = CPU 模型 stage） | both | 🔧(heuristic + `model_stage.fasttext_scorer` 接 lid.176,CPU 模型 stage) |

### L2 —— 去重（预训练最硬的问题）
| L2.1 | 全局模糊去重（MinHash-LSH 跨**整语料**万亿文档） | pre | ✅(默认流式 union-find;`cc_workers>0` 走**分布式 label-propagation**(`jude.dist_cc`):标签数组按 rid%W 分片到 actor,每轮定向消息 shuffle 取 min,driver 只留 {rid→更小 rep} 稀疏映射,不再持有全量标签 —— 与单机 parity) |
| L2.2 | 全局精确子串去重 | pre | ✅(`dist_substring_dedup`:shuffle k-窗口哈希→每 hash 全局 min-(doc,pos) 保留者→其余标记删除→driver 按 doc 去重叠 token;与单机 `substring_dedup` 逐行一致,共享 `window_hashes` 保证 parity) |
| L2.3 | 增量去重（新 CC dump 对**已存在语料**去重,不重跑全量）—— 需持久化 dedup 索引(bloom/MinHash store) | pre | 🔧(jude.incremental_dedup.HashIndex:持久化 content-hash 索引(Lance,版本化),增量运行 O(新文档);exact 已做,fuzzy/MinHash band 索引为后续) |
...
| L3.3 | perplexity 过滤（KenLM 或小 LM stage） | pre | 🔧(`model_stage.kenlm_perplexity_scorer` + model_filter(keep="<="),CPU) |
...
| L4.5 | tokenizer-aware 长度阈值（质量门用 token 数而非词数，C15） | both | ✅(curate.token_length_filter) |
...
| X.2 | shuffle spill（预训练全局 shuffle/dedup **必须** spill） | pre | 🔧(SQL reducer 走 DuckDB 自带 spill:worker 连接 `SET temp_directory`+`memory_limit`,env 开;非自建 spill 引擎。纯 Python shuffle 的 fuzzy-dedup 已改流式 L2.1) |
| X.3 | 交互式采样 + 被丢文档探查（`sample_dropped`，调阈值救命） | both | ✅ |
| X.4 | 规模真凭实据（当前 bench 单机/模拟多机，PB/百节点无实证） | both | 🔧(端到端 pipeline harness `bench_pretraining_pipeline`:extract→lid→quality→dedup→tokenize→shard + funnel/吞吐;真 PB/多机数待真实集群) |
