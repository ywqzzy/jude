...
- 🔧 **C1 web 管线前半** → 大部分修:`c4_line_filter`(行级清洗)+ `corpus_line_dedup`(跨文档行去重)+ **`substring_dedup`**(精确子串去重,Lee et al. 的实用滚动哈希形式:≥k token 的重复段落在首次出现后被移除,即使周围文档不同——文档级去重抓不到的共享段落/许可头/引文)。测试 `test_web_curation.py`(7)+ `test_substring_dedup.py`(5)。仍待办:HTML/WARC 抽取、完整后缀数组(当前是 O(n) 滚动哈希近似,极小概率哈希碰撞会多删一段)。
